...
MYSQLROOT=$(aws secretsmanager get-secret-value --secret-id "evlab.ch/glpi/mysqlroot" --output json|jq -r '.SecretString'|jq -r '.mysqlroot')
MYSQLUSER=$(aws secretsmanager get-secret-value --secret-id "evlab.ch/glpi/mysqluser" --output json|jq -r '.SecretString'|jq -r '.mysqlroot')
MYSQLDB=glpidb
